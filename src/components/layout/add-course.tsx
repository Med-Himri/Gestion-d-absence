...
//                              onClick={handleSave}
//                              type="submit" 
//                              className="bg-teal-700 dark:hover:text-neutral-950 dark:text-neutral-300">Enregistrer</Button>
//                         </div>
//                     </DialogFooter>
//                 </DialogContent>
...
import { useTeachersStore } from "@/store/teacher-store";
import { supabase } from '@/config/supabase'; // Make sure supabase is imported
...
export function FormCourse() {
    ...
    const handleSave = async () => {
        if (!courseName.trim()) {
            alert("⚠️ Veuillez entrer le nom du cours");
            return;
        }
        if (selectedTeachers.length === 0) {
            alert("⚠️ Veuillez sélectionner au moins un enseignant");
            return;
        }
        ...
        try {
            ...
            // Assign teachers to course (assuming you have a table like teacher_course)
            const assignments = selectedTeachers.map((teacherId) => ({
    ...
    return (
        <div>
            <Dialog>
                ...
                <DialogContent className="dark:bg-neutral-900 sm:max-w-[440px] max-h-[80vh]">
                    <DialogHeader>
                        <DialogTitle className="text-neutral-900 dark:text-neutral-300">Nouveau Cours</DialogTitle>
                    </DialogHeader>

                    <div className="flex items-center gap-2 mb-1 mt-4">
                        <BookOpenText className="w-5 h-5 text-blue-900" />
                        <span className="text-sx text-neutral-950 font-medium dark:text-neutral-300">Informations du cours</span>
                    </div>
                    ...
                    <div>
                        ...
                        <Popover open={open} onOpenChange={setOpen}>
                            ...
                            >
                                ...
                                <div className="max-h-60 overflow-y-auto">
                                    {filteredTeachers.length === 0 ? (
                                        <div className="p-3 text-sm text-gray-500 dark:text-neutral-400">
                                        ...
                                        filteredTeachers.map((teacher) => (
                                            <div
                                                key={teacher.user_id}
                                                onClick={() => toggleTeacher(teacher.user_id)}
                                                className={`flex items-center gap-3 px-3 py-2 cursor-pointer hover:bg-neutral-100 dark:hover:bg-neutral-800 ${
                                                    selectedTeachers.includes(teacher.user_id)
                    ...
                    <hr />
                    <DialogFooter>
                        <span className="fixed text-xs left-6 text-gray-500">* Champs obligatoires</span>
                        <div className="flex space-x-2">
                            <DialogClose asChild>
                                <Button variant="outline" className="dark:hover:bg-neutral-100 dark:hover:text-neutral-950">
                                    Annuler
                                </Button>
                            </DialogClose>
                            <Button
                                onClick={handleSave}
                                className="bg-teal-700 dark:hover:text-neutral-950 dark:text-neutral-300"
                            >
                                Enregistrer
                            </Button>
                        </div>
