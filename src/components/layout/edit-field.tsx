...
interface EditFieldProps {
    field: any // You can replace this with your Teacher type if you want
}

export function EditField({ field }: EditFieldProps) {
    ...
    useEffect(() => {
        if (field) {
            setForm({ name: field.name });

            // ✅ Preselect groups by ID
            if (field.groups && Array.isArray(field.groups)) {
                const groupIds = field.groups.map((g: any) => g.id);
